...
`,
  ...
  methods: {
    ...
    like(){
      ...
      this.axios.post(`/tx/v2.0/transactions/${this.txAction.like}`,{
          'txId': txId,
          'from': this.wallet.pd.address,
          'inputs' : {
            '_to': this.wallet.user.address,
            '_value': '100' + this.stDec
          }
      },
      {
        headers: {
    ...
    fund(){
      ...
      const txId = this.uuidv4();
      window.txIds.push(txId);
      this.axios.post(`/tx/v2.0/transactions/${this.txAction.funding}`,{
            'txId': txId,
            'from': this.wallet.pd.address,
            'inputs' : {
              '_to': this.wallet.user.address,
              '_value': '1000' + this.stDec
            }
        },
        {
          headers: {
